...
if __name__ == '__main__':
    ...
    nois = [1, 2, 7, 9, 10, 11, 15, 16, 20, 23, 28] # Start at 0
    ...
    exists, resultsdir = fslnets.make_dir(datadir,
                                        'fslnets_results') # make results directory
    all_ts = [] # Create empty list to hold timeseries data

    ## Loop over subjects to get cleaned data (artifactual nodes regressed out)     
    ...
    ## Loop over network correlation measures, run on each subject individually    
    for stat in net_measures:
        all_netmat = np.zeros((nsubs, nnodes*nnodes)) # Create empty array to hold results
        # Loop over subjects
        for i in range(len(sublist)):
            subj = sublist[i]  
